class Solution:
    ...
    def criticalConnections(self, n: int, connections: List[List[int]]) -> List[List[int]]:
        ...
        # please note this rank is NOT the num (name) of the vertex itself, it is the order of your DFS level
        currentRank = 0

        # here lowestRank[i] represents the lowest order of vertex that can reach this vertex i
        ...
        # It will be used in the following DFS because we need to know where the current DFS level comes from. You do
        # not need to setup this parameter, I setup here ONLY because it is more clear to see what are passed on in
        # the DFS method.
    ...
    def _dfs(self, res, graph, lowestRank, visited, currentRank, prevVertex, currentVertex):
        ...
        for nextVertex in graph[currentVertex]:
            if nextVertex == prevVertex:
                continue  # do not include the the incoming path to this vertex since this is the possible ONLY
                # bridge (critical connection) that every vertex needs.

            if not visited[nextVertex]:
                self._dfs(res, graph, lowestRank, visited, currentRank + 1, currentVertex, nextVertex)
            # We avoid visiting visited nodes here instead of doing it at the beginning of DFS - the reason is,
            # even that nextVertex may be visited before, we still need to update my lowestRank using the visited
